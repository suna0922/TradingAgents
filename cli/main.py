...
def save_report_to_disk(final_state, ticker: str, save_path: Path):
    ...
    # 0. L1 Raw Report (机器生成原始报告，用于审计 LLM 幻觉)
    l1_raw_dir = save_path / "0_l1_raw"
    # 尝试从 run_l1_analysis 自动保存的位置复制 L1 原始报告
    from tradingagents.default_config import DEFAULT_CONFIG
    _l1_auto_path = Path(DEFAULT_CONFIG["results_dir"]) / ticker / "0_l1_raw" / "l1_structured_report.md"
    if _l1_auto_path.exists():
        l1_raw_dir.mkdir(exist_ok=True)
        import shutil
        shutil.copy2(_l1_auto_path, l1_raw_dir / "l1_structured_report.md")
