...
class NormalizedChatOpenAI(ChatOpenAI):
    ...
    def invoke(self, input, config=None, **kwargs):
        try:
            ...
            err_str = str(e)
            import logging
            _dlog = logging.getLogger("deepseek_diag")
            # 记录所有异常（不只 400），方便定位
            try:
                import json
                msgs = _input_to_messages(input) if True else []
                total_input = 0
                for m in msgs:
                    total_input += len(json.dumps(m.model_dump() if hasattr(m, 'model_dump') else str(m), ensure_ascii=False))
                is_400 = "400" in err_str or "input length" in err_str.lower() or "context_length" in err_str.lower()
                log_fn = _dlog.error if is_400 else _dlog.warning
                log_fn(
                    "[INVOKE-ERROR] model=%s | is_400=%s | input_msgs=%d | input_chars≈%d | error=%s",
                    getattr(self, 'model_name', self.model if hasattr(self, 'model') else '?'),
                    is_400, len(msgs), total_input, err_str[:800],
                )
            except Exception:
                _dlog.error("[INVOKE-ERROR] (detail capture failed) error=%s", err_str[:500])
            raise
...
class DeepSeekChatOpenAI(NormalizedChatOpenAI):
    ...
    def _get_request_payload(self, input_, *, stop=None, **kwargs):
        ...
        # ── 诊断日志: 记录每次 API 调用的输入大小 ──
        try:
            import json
            total_chars = len(json.dumps(payload, ensure_ascii=False))
            msg_count = len(outgoing)
            reasoning_total = 0
            tool_result_total = 0
            for md in outgoing:
                rc = md.get("reasoning_content", "")
                if rc:
                    reasoning_total += len(str(rc))
                role = md.get("role", "")
                content = md.get("content", "")
                if role == "tool":
                    tool_result_total += len(str(content))

            # 使用 logging 而非 print，避免干扰正常输出
            import logging
            logger = logging.getLogger("deepseek_diag")
            logger.warning(
                "[DEEPSEEK-API-DIAG] model=%s | messages=%d | payload_chars=%d | "
                "reasoning_chars=%d | tool_result_chars=%d | est_tokens≈%d",
                getattr(self, 'model_name', self.model if hasattr(self, 'model') else '?'),
                msg_count, total_chars, reasoning_total, tool_result_total,
                int(total_chars * 1.2),  # 粗估 token
            )
        except Exception:
            pass  # 诊断日志不能影响正常流程
